...
const Search = () => {
  ...
  return (
    <Layout>
      <SEOHead title={hasQuery ? `Search: ${query}` : "Search"} description="Search VBB STORE for products, blog posts, and guides." noIndex />
                    ...
                    {results.products.map((p) => (
                      <Link key={p.id} to={`/product/${p.slug}`} className="flex items-center gap-4 p-4 bg-card border border-border rounded-xl hover:shadow-md transition-shadow">
                        <div className="w-16 h-16 rounded-lg bg-secondary overflow-hidden shrink-0">
                          {p.image_url ? <img src={p.image_url} alt={p.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-xs text-muted-foreground">{p.category}</div>}
                        </div>
                  ...
                  <div className="space-y-3">
                    {results.posts.map((post) => (
                      <Link key={post.id} to={`/blog/${post.slug}`} className="flex items-center gap-4 p-4 bg-card border border-border rounded-xl hover:shadow-md transition-shadow">
                        <div className="w-16 h-16 rounded-lg bg-secondary overflow-hidden shrink-0">
                          {post.featured_image ? <img src={post.featured_image} alt={post.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-xs text-muted-foreground">{post.category}</div>}
                        </div>
                        <div className="flex-1 min-w-0">
